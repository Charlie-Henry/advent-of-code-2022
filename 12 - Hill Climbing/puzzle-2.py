...
starting_locs = find_starting_positions(grid)

least_steps = len(grid) * len(grid[0])
for loc in starting_locs:
    res = dijkstras(grid, loc, goal)
    if res is not None:  # possible no answer is found
        if res < least_steps:
            least_steps = dijkstras(grid, loc, goal)

print(least_steps)  # 386
