...
def move_crate_multi(crates, quantity, origin, destination):
    queue = []
    for i in range(0, quantity):
        queue.insert(0, crates[origin].pop())

    crates[destination] = crates[destination] + queue
    return crates


for item in instructions:
    stacks = move_crate_multi(stacks, item[0], item[1], item[2])

ans = str()
for i in range(1, 10):  # Answers output
    ans = f"{ans}{stacks[i].pop()}"

print(ans)  #  Part 2: RMHFJNVFP
